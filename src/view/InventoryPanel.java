...
public class InventoryPanel extends JPanel {
    ...
    private void openProductDialog(Product existing) {
        ...
        btnSave.addActionListener(e -> {
            ...
            boolean ok = isEdit ? dao.updateProduct(p) : dao.addProduct(p, loggedInUser.getUserId());
            if (ok) {
                loadProducts();
                dialog.dispose();
                showToast(isEdit
    ...
    private void deleteSelected() {
        ...
        panel.add(new JLabel("<html>You are about to dispose of <b>" + name +
                "</b>.<br>This will log it to Disposed Items and remove it from inventory.</html>"),
                BorderLayout.NORTH);
        JTextField reasonField = new JTextField(28);
        reasonField.setBorder(BorderFactory.createTitledBorder("Reason (required)"));
        panel.add(reasonField, BorderLayout.CENTER);

        int confirm = JOptionPane.showConfirmDialog(this, panel,
                "Confirm Disposal", JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE);
        if (confirm != JOptionPane.OK_OPTION) return;

        String reason = reasonField.getText().trim();
        if (reason.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Please enter a reason for disposal.",
